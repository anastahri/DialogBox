...
class Thread extends Eloquent
{
    ...
    */
     ...
     * @return string
     */
    public function participantsString2($userId = null, $columns = ['name'])
    {
        $participantsTable = Models::table('participants');
        $usersTable = Models::table('users');
        $userPrimaryKey = Models::user()->getKeyName();
        ...
        if ($participantNames->count() > 2) {
            $others = $participantNames->count() - 2;
            $participantNames = $participantNames->get()->slice(0,2);
            $participantNames = $participantNames->implode('name', ', ');
            $participantNames .= ' and '. $others .' more';
